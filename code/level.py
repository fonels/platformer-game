...
class Level:
    def __init__(self, tmx_map, level_frames, audio_files, data, switch_stage, text_system):
    ...
    def setup(self, tmx_map, level_frames, audio_files):
        ...
        for obj in tmx_map.get_layer_by_name('BG details'):
            if obj.name == 'static':
                ...
                print(f"Debug: Creating AnimatedSprite with frames for {obj.name}: {frames}")  # Debug log
                if not frames:
                    print(f"Warning: No frames found for {obj.name}")
                    continue  # Skip if frames is empty
                AnimatedSprite((obj.x, obj.y), frames, self.all_sprites, Z_LAYERS['bg tiles'])
                if obj.name == 'candle':
    ...
    def check_constraint(self):
        ...
        if self.player.hitbox_rect.colliderect(self.level_finish_rect):
            ...
            level_time = end_time - self.start_time
            self.data.add_level_time(level_time, self.data.current_level)
    ...
    def run(self, dt):
        ...
        if self.data.current_level == 0:
            if self.data.coins > 0 and not hasattr(self, 'coin_message_shown'):
                self.text_system.add_message("Молодец! Монеты дают очки и дополнительные жизни", 1000)
                self.coin_message_shown = True

            if self.data.health < 3 and not hasattr(self, 'damage_message_shown'):
                self.text_system.add_message("Осторожно! Враги и шипы отнимают жизни", 1000)
                self.damage_message_shown = True